...
public interface SparkBaseController {
    ...
    ResponseEntity<String> createSpark(@AuthenticationPrincipal UserDetails authentication, @RequestBody String sparkContent);

    @PutMapping(path = "",
            consumes = MediaType.APPLICATION_JSON_VALUE,
            produces = MediaType.APPLICATION_JSON_VALUE)
    @Operation(summary = "UPDATE SPARK", security = @SecurityRequirement(name = "Bearer Authentication"))
    ResponseEntity<String> updateSpark(@AuthenticationPrincipal UserDetails authentication, @RequestParam UUID id, @RequestBody String newContent);

    @DeleteMapping(path = "")
    @Operation(summary = "DELETE SPARK", security = @SecurityRequirement(name = "Bearer Authentication"))
    ResponseEntity<String> deleteSpark(@AuthenticationPrincipal UserDetails authentication, @RequestParam UUID id);
}